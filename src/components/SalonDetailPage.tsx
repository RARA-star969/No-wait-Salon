...
import { ArrowLeft, Bookmark, Brush, CalendarDays, Check, CheckCircle2, ChevronDown, ChevronRight, Clock, Clock3, CreditCard, ExternalLink, Info, MapPin, Navigation, Palette, PhoneCall, Plus, ScanFace, Scissors, Share2, Sparkles, Store, Tag, Timer, Waves, Wifi, Wind, X } from 'lucide-react';
...
export const SalonDetailPage: React.FC<Props> = ({ salon, nearbySalons, queue, barbers, selectedService, setSelectedService, selectedServiceIds, setSelectedServiceIds, appliedOfferId, onApplyOffer, onRemoveOffer, onBack, onJoin, onReserve, userEntry }) => {
  ...
  return (
    <div id="customer-salon-screen" className="relative min-h-full overflow-y-auto bg-[#F5F7F6] pb-[calc(8.5rem+env(safe-area-inset-bottom))] text-[#17201F] animate-in fade-in duration-200">
      {/* Signature floating capsule: a top-center notch/island, content-hugging
          ...
          there, taking the capsule with it). */}
      <div
      ...
      {/* Sticky action dock. Deliberate departure from APK45: no background
          slab behind the buttons — they float directly on the page with
          just safe-area padding and their own shadows. The selected-service
          summary stays a separate mounted-but-collapsing card above the
          buttons (the already-approved treatment), never folded into the
          Join Queue button's label. */}
      <div className="fixed inset-x-0 bottom-0 z-30 px-3 pt-2.5 pb-[max(.75rem,env(safe-area-inset-bottom))]">
        <div className="mx-auto max-w-xl">
          <div className={`grid overflow-hidden transition-[grid-template-rows,opacity,margin-bottom] duration-300 ease-out ${totals.count > 0 ? 'mb-2 grid-rows-[1fr] opacity-100' : 'grid-rows-[0fr] opacity-0'}`}>
            <div className="min-h-0">
              <button
                type="button"
                id="dock-summary-btn"
                onClick={() => setBreakdownOpen(true)}
                className={`flex w-full items-center justify-between gap-3 rounded-2xl border border-[#E0E7E6] bg-white/95 px-4 py-2.5 text-left shadow-[0_14px_28px_-16px_rgba(23,32,37,0.35)] backdrop-blur-md ${dockBounce ? 'dock-bounce' : ''}`}
              >
                <span className="min-w-0">
                  <span className="block text-[11px] font-semibold text-[#4C5A58]">{totals.count} {totals.count === 1 ? 'service' : 'services'} selected · {totals.totalDurationMin} min session</span>
                ...
                <span className="flex shrink-0 items-center gap-1.5">
                  {dockDiscountInr > 0 ? (
                    <span className="flex items-baseline gap-1.5">
                      ...
                      <span className="text-sm font-bold text-[#17201F]">₹{dockFinalTotalInr}</span>
                    </span>
                  ) : (
          ...
          <div className="grid grid-cols-[1fr_auto] gap-2">
            <button
              ...
              onClick={onJoin}
              className={`flex min-h-13 min-w-0 items-center justify-center rounded-2xl bg-[#0F766E] px-3 text-xs font-bold text-white shadow-[0_10px_20px_-10px_rgba(15,118,110,0.6)] transition active:scale-[0.98] sm:text-sm ${dockBounce ? 'dock-bounce' : ''}`}
            >
              <span className="min-w-0 truncate">{userEntry ? 'View live queue' : 'Join Queue'}</span>
            </button>
            <button id="reserve-slot-btn" onClick={onReserve} aria-label="Reserve a future queue window" className="flex min-h-13 w-13 shrink-0 items-center justify-center rounded-2xl border border-[#BFD6D2] bg-white text-[#0F766E] shadow-[0_10px_22px_-14px_rgba(23,32,31,0.3)] transition active:scale-[0.98]"><CalendarDays className="h-5 w-5" /></button>
          </div>
        </div>
      </div>
