...
export const GymMemberHub: React.FC<{
  ...
  return (
    <div className="fixed inset-0 z-[95] flex flex-col bg-[#F8FAFA]">
      <header className="flex items-center gap-3 border-b border-[#E1E7E6] bg-white px-4 py-3 pt-[max(0.75rem,env(safe-area-inset-top))]">
        <button onClick={onClose} aria-label="Back to Gym Activity" className="flex h-9 w-9 items-center justify-center rounded-full bg-[#F0F5F4] text-[#17201F]">
          <ArrowLeft className="h-4 w-4" />
        </button>
        <div className="min-w-0 flex-1">
          <h1 className="truncate text-sm font-bold text-[#17201F]">{gymName}</h1>
        ...
        <div className="flex-1 overflow-y-auto px-4 py-4 pb-[max(1.5rem,env(safe-area-inset-bottom))] [-webkit-overflow-scrolling:touch]">
          <div className="mx-auto max-w-md space-y-3">
            {membership && (
              <div className="rounded-2xl border border-[#E0E7E6] bg-white p-4">
                <div className="flex items-center justify-between">
                  <span className="text-xs font-bold text-[#17201F]">Membership details</span>
            ...
            >
              ...
              <div className="min-w-0 flex-1">
                <p className="text-xs font-bold text-[#17201F]">Workout Plan</p>
            ...
            >
              ...
              <div className="min-w-0 flex-1">
                <p className="text-xs font-bold text-[#17201F]">Attendance Calendar</p>
            ...
            {data?.attendance && (
              <div className="grid grid-cols-2 gap-3">
                <div className="rounded-2xl border border-[#E0E7E6] bg-white p-4">
                  ...
                  <p className="mt-1 text-lg font-extrabold text-[#17201F]">{data.attendance.currentStreak} <span className="text-xs font-semibold text-[#71807E]">days</span></p>
                </div>
                <div className="rounded-2xl border border-[#E0E7E6] bg-white p-4">
                  ...
                  <p className="mt-1 text-lg font-extrabold text-[#17201F]">{data.attendance.avgVisitsPerWeek}</p>
                </div>
              </div>
            )}
            ...
            {(data?.recentVisits?.length ?? 0) > 0 && (
              <div className="rounded-2xl border border-[#E0E7E6] bg-white p-4">
                <p className="text-[10px] font-extrabold uppercase tracking-wider text-[#6F7C7A]">Visit history</p>
                <div className="mt-2 space-y-2">
                  {data!.recentVisits.slice(0, 8).map((visit) => (
                    <div key={visit.id} className="flex items-center justify-between border-b border-[#EDF1F0] pb-2 text-[11px] last:border-0 last:pb-0">
                      ...
                      <span className="font-semibold text-[#17201F]">{gymVisitDurationLabel(visit, Date.now())}</span>
                    </div>
                  ))}
                </div>
              </div>
