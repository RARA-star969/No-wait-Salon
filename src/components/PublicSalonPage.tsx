...
const MetricCard: React.FC<{ label: string; value: string; hint?: string }> = ({ label, value, hint }) => (
  <div className="rounded-2xl border border-[#E2EAE9] bg-white p-3.5">
    <p className="text-[10px] font-bold uppercase tracking-[0.1em] text-[#8B9795]">{label}</p>
    <p className="mt-1 text-[19px] font-extrabold leading-tight text-[#17201F]">{value}</p>
    {hint && <p className="mt-0.5 text-[11px] text-[#8B9795]">{hint}</p>}
  </div>
);
...
export const PublicSalonPage: React.FC<{ token: string }> = ({ token }) => {
  ...
  const [barbersTotal, setBarbersTotal] = useState(1);
  ...
  useEffect(() => {
    ...
    const apply = (state: { queue: QueueItem[]; barbers?: Array<{ status: string }>; completedList?: QueueItem[] }) => {
      ...
      if (state.barbers) {
        ...
        setBarbersTotal(state.barbers.length || 1);
      }
      setEntry((current) => {
  ...
  const chairsReady = barbersActive;
      ...
      <div className="mx-auto w-full max-w-[26rem]">
        ...
        <main className="px-4 pb-[calc(env(safe-area-inset-bottom)+2rem)] pt-4">
          ...
          <div className="mt-4 grid grid-cols-2 gap-3">
            <MetricCard
              label="Live wait"
              value={isQueued ? estimatedWait : waitLabel(profile.liveWaitMinutes) === 'No wait right now' ? 'No wait' : waitLabel(profile.liveWaitMinutes).replace(' wait', '')}
              hint={isQueued ? undefined : 'approx.'}
            />
            <MetricCard label="People ahead" value={String(isQueued ? peopleAhead : waiting)} />
          </div>
          ...
          {step === 'salon' && (
            <>
              <div className="mt-4 rounded-2xl border border-[#CFE6E2] bg-[#EAF6F4] p-4">
                <p className="flex items-center gap-1.5 text-[11px] font-bold text-[#0F766E]">
                  <Radio className="h-3.5 w-3.5" /> Live queue update
                </p>
                <p className="mt-1.5 text-[19px] font-extrabold leading-tight text-[#0B4A44]">
                  {waitLabel(profile.liveWaitMinutes) === 'No wait right now' ? 'No wait right now' : `${waitLabel(profile.liveWaitMinutes)}`}
                </p>
                <p className="mt-1 flex items-center gap-1.5 text-xs text-[#4F7F7A]">
                  <User className="h-3.5 w-3.5" /> {waiting} people ahead · {chairsReady} of {barbersTotal} chairs ready
                </p>
              </div>
              ...
              <div className="mt-6 flex items-end justify-between">
                <div>
                  <p className="text-[10px] font-bold uppercase tracking-[0.14em] text-[#8B9795]">Choose your service</p>
                  <h2 className="mt-0.5 text-[16px] font-extrabold">Service menu</h2>
                </div>
                <span className="text-xs font-semibold text-[#0F766E]">Select one service</span>
              </div>
