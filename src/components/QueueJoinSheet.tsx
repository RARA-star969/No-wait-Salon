...
import type { Barber, Salon, ServiceItem } from '../types';
import { buildJoinPreview } from '../shared/liveTicket';
...
type Props = {
  ...
  barbers: Barber[];
  queue: Parameters<typeof buildJoinPreview>[0];
  busy?: boolean;
  error?: string;
      ...
      >
        ...
        <div className="mt-4 min-h-0 flex-1 overflow-y-auto px-5">
          ...
          <section className="mt-4 rounded-2xl border border-[#E1E7E6] bg-white p-4">
            <p className="text-[10px] font-bold uppercase tracking-[0.16em] text-[#73827F]">Your service</p>
            <div className="mt-2 flex items-start justify-between gap-3">
              <div className="min-w-0">
                <p className="truncate text-sm font-bold text-[#17201F]">{service?.name || 'Service'}</p>
                {service?.description && (
                  <p className="mt-0.5 line-clamp-2 text-[11px] leading-4 text-[#788582]">{service.description}</p>
                )}
                {service?.durationMin ? (
              ...
              {service?.priceInr ? <span className="shrink-0 text-base font-bold text-[#17201F]">₹{service.priceInr}</span> : null}
            </div>
          </section>
